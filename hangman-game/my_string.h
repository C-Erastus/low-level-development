...
//Preconditon: c_string is a valid null terminated string.
//Postcondition: Allocate space for a string object that represents a string
// with the smae value as the given c_string. the capacity  of the string 
// object will be set to be one greater than is required to hold the string 
// As an example, the string "the" would set capactiy to 4 instead of 3. A
// copy of the address of the opaque object will be returned on success and 
// NULL on failure
MY_STRING my_string_init_c_string(const char *c_string);

//Preconditon: hMy_string is the handle of a valid My_string object
...
//Precondtion: hLeft_string and hRight_string are valid My_string objects.
//Postcondition: returns an integer less than zero if the string represented
// by hLeft_string is lexicographicaly smaller than hRight_string. If
// one string is aprefix of the other string then the shorter string is 
// considered to be the smaller one. (app is less than apple). Returns
// 0 if the strings are the smae and returns a number greater than zero 
// if the string represented by hLeft_string is bigger than hRight_string string. 
int my_string_compare(MY_STRING hLeft_string, MY_STRING hRight_string);

//Preconditon: hMy_string is the handle to a valid My_string object.
//Postconditon: hMy_string will be the handle of a string object hat contains
// the next string from the file stream fp according to the following rules.
// 1) Leading whitespace will be ignored
// 2) All character (after the fist non-whitespace character is obtained 
//    and included) will be added to the string unil a stopping conditon
//	  is met. The capacity of the string will continue to grow as needed
//	  until all characters are stored
// 3) A stopping conditon is met if we read a whitespace character after
...
// and FAILURE otherwise. remember that the incoming string my already
// contain some data and this function should replace the data but not
// necessarily resize the array unless needed 
Status my_string_extraction(MY_STRING hMy_string, FILE *fp);

//Preconditon: hMy_string is the handle to a valid My_string object.
//Postconditon: Writes the characters contained in the string object indicated
// by the handle hMy_string to the file strea fp.
...
// the cost of a resize by making the string capacity somewhare larger than 
// it was before (up to 2 times bigger)
Status my_string_push_back(MY_STRING hMy_string, char item);

//Precondition: hMy_string is the handle to a valid My_string object.
//Postconditon: Removes the last character of a string in constant time.
// Guaranteed to not cause a resize operation of the internal data. Retruns 
...
// is not usalbe as a c-string since the data is not NULL terminated nd is 
// intended to just proved access to the element at that index. 
char *my_string_at(MY_STRING hMy_string, int index);

//Preconditioni: hMy_string is the handle to a valid My_string object. 
//Postconditioni: returns the address of the first element of the string object
// for use as a C-string. The resulting c-string is guaranteed to tbe NULL 
// terminated and the memeory is still maintained by the string object through
...
// hResult object followed by the hAppend object concatenated togethe. This
...
// is unable to accomodate the characters int he hAppend string perhaps
// because of the failed resize opertion. On FAILURE, no change to either 
// string should be made
Status my_string_concat(MY_STRING hResult, MY_STRING hAppend);

//Preconditon: hMystring is the hangle to a vaild My_string object
...
// pLeft is originally NULL then the function willa ttempt to initialize
...
// left as NULL and any memory that may have geen used by a potential 
